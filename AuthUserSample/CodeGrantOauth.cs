...
namespace AuthUserSample
{
    public class CodeGrantOauth : Form
    {
        ...
        public static string scopes = ConfigurationManager.AppSettings.Get("Scope");

        private string AuthorizationUri = env + "/apisecurity/connect/authorize";  // Authorization code endpoint
        private string RedirectUri = redirect;  // Callback endpoint
        ...
        private string CodeQueryString = "?client_id={0}&client_secret={1}&scope=" + scopes + "&response_type=code&redirect_uri={2}";
        private string AccessBody = "client_id={0}&client_secret={1}&code={2}&grant_type=authorization_code&redirect_uri={3}";
        ...
        public CodeGrantOauth(string clientId)
        {
            ...
            this._uri = string.Format(this.AuthorizationUri + this.CodeQueryString, this._clientId, this._clientSecret, RedirectUri);
        }
        ...
        public string GetAccessToken()
        {
            ...
            try
            {
                ...
                {
                    var accessTokenRequestBody = string.Format(this.AccessBody, this._clientId, this._clientSecret, this._authorizationCode, WebUtility.UrlEncode(RedirectUri));
        ...
        public string RefreshAccessToken(string refreshToken)
        {
            ...
            try
            {
                var refreshTokenRequestBody = string.Format(this.RefreshBody, this._clientId, this._clientSecret, WebUtility.UrlEncode(RedirectUri), refreshToken);
        ...
        private void browser_Navigated(object sender, WebBrowserNavigatedEventArgs e)
        {
            ...
            if (e.Url.AbsoluteUri.Contains(RedirectUri))
            {
